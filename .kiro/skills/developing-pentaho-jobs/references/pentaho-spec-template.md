...
> in design before building. Static acceptance is the build completion boundary;
> runtime execution is a separate, phase-gated criterion (see the two acceptance
> sections at the end). Business-data correctness is **never** proven here.
...
Business-data correctness is explicitly **not** proven by these criteria.

## Runtime acceptance criteria

Runtime verification is **optional and phase-gated**: it runs only after static
acceptance passes, and `kettle_runtime_execute` additionally requires explicit
user approval and a detected `PENTAHO_HOME`. State, per artifact that may be
verified at runtime:

- **Loadcheck expectation:** `kettle_runtime_loadcheck` returns `PASS` (Kitchen/
  Pan loads the artifact) — allowed only after static acceptance.
- **Execute expectation (when authorized):** the parameters to pass (external,
  no literal credentials), and the expected terminal status (`PASS`).
- **Not authorized:** if runtime execution is not approved, record it as
  deferred verification in the handoff.

Runtime failure (`FAIL`/`TIMEOUT`/`STATIC_VALIDATION_FAILED`/`UNAVAILABLE`) is
never reported as a successful implementation.
